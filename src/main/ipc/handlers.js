...
export function registerIpcHandlers() {
    ipcMain.handle('process-artifact', async (event, data) => {
        try {
            ...
            return new Promise((resolve, reject) => {
                db.run(`INSERT INTO artifacts (id, type, source, extracted_content, ai_recommendation, ai_reasoning, provider, model, created_at, updated_at)
    ...
    ipcMain.handle('get-artifacts', async () => {
        const db = getDatabase();
        return new Promise((resolve, reject) => {
            db.all('SELECT * FROM artifacts ORDER BY created_at DESC', (err, rows) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    ipcMain.handle('get-artifact', async (event, id) => {
        const db = getDatabase();
        return new Promise((resolve, reject) => {
            db.get('SELECT * FROM artifacts WHERE id = ?', [id], (err, row) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    ipcMain.handle('reprocess-artifact', async (event, id) => {
        ...
        // Update the artifact
        return new Promise((resolve, reject) => {
            db.run(`UPDATE artifacts 
         SET ai_recommendation = ?, ai_reasoning = ?, provider = ?, model = ?, updated_at = datetime('now')
         WHERE id = ?`, [result.recommendation, result.reasoning, result.provider, result.model, id], function (err) {
                if (err) {
                    reject(err);
                }
                else {
